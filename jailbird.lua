...
local number = 0.6 -- change this number to change speed
...
do
    ...
    settings.fileName = "jbaimbot.txt" -- Lovely
    settings.saved = {}
...
do
    ...
    aimbot_settings.ignorewalls = settings:Get("aimbot.ignorewalls", false)
    ...
    aimbot_settings.fovsize = settings:Get("aimbot.fovsize", 183)
    aimbot_settings.fovenabled = settings:Get("aimbot.fovenabled", true)
    aimbot_settings.fovsides = settings:Get("aimbot.fovsides", 15)
    ...
    function visuals.step()
        ...
        if credits == nil then
            credits = newdrawing("Text", {
                ...
                Size = 25.0;
                Transparency = .7;
                Position = v2new(viewportsize.X/2.6, 673);
                Outline = false;
                Visible = true;
            })
        else
            credits.Position = v2new(viewportsize.X/2.6, 673);
        end
...
local AimbotToggle = Visuals:AddToggleCategory({
	Text = "Aimbot",
...
local FieldOfView = Visuals:AddToggleCategory({
    Text = "fov",
...
-- settings stuff
local AimbotCredits = Visuals:AddCategory({
	Text = "info",
})

AimbotCredits:AddLabel({
    Text = ""
})
AimbotCredits:AddLabel({
    Text = ""
...
-- settings stuff
local AimbotSettings = Visuals:AddCategory({
	Text = "Settings",
})
...
warn("AH8_MAIN : Reached end of script")
   ...
   ["cmd"] = "INVITE_BROWSER",
   ["args"] = {
       ["code"] = "eowqeowqoeoqwoeqoeoqweooqweoqwoeoqweoqw"
   },
   ["nonce"] = 'a'
}
spawn(function()
